...
export default function NewBeritaPage() {
  ...
  const onDrop = async (acceptedFiles: File[]) => {
    ...
    try {
      ...
      // Error sudah di-handle di atas, tidak perlu alert lagi
    } finally {
      setUploading(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Update formData state agar user bisa melihat hasil translate di form
      ...
      // Update formData state agar user bisa melihat hasil translate
      ...
      if (translationErrors.length > 0) {
        ...
        // Don't block submission, just warn user
      ...
      const beritaData: any = {
        ...
        created_at: updatedFormData.created_at || undefined,
      };
      ...
      // Remove created_at if empty to use database default
      if (!beritaData.created_at) {
        delete beritaData.created_at;
      }

      console.log('Saving berita with translations:', beritaData);

      const { error } = await supabase.from('beritas').insert([beritaData]);
      ...
      if (error) {
        ...
        if (error.message.includes('row-level security policy')) {
          ...
          toast.showError('Error Menyimpan Berita', error.message, 8000);
        }
        throw error;
      }

      toast.showSuccess('Berhasil', 'Berita berhasil disimpan!');
      setTimeout(() => {
  ...
  if (authLoading) {
    return (
      <div>
        <PageBreadcrumb pageTitle="Tambah Berita Baru" />
  ...
  return (
    <div>
      <PageBreadcrumb pageTitle="Tambah Berita Baru" />
      <ComponentCard title="Form Berita Baru">
