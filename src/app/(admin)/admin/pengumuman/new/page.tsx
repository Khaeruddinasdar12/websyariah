...
export default function NewPengumumanPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.slug) {
      toast.showError('Slug Kosong', 'Slug tidak dapat dibuat dari judul');
      return;
    }
    ...
    try {
      ...
      if (!formData.judul_en || !formData.konten_en) {
        const [judulEn, kontenEn] = await Promise.all([
          formData.judul ? translateText(formData.judul, 'en') : '',
          formData.konten ? translateText(formData.konten, 'en') : '',
        ]);
        formData.judul_en = judulEn || formData.judul_en || '';
        formData.konten_en = kontenEn || formData.konten_en || '';
      }

      if (!formData.judul_ar || !formData.konten_ar) {
        const [judulAr, kontenAr] = await Promise.all([
          formData.judul ? translateText(formData.judul, 'ar') : '',
          formData.konten ? translateText(formData.konten, 'ar') : '',
        ]);
        formData.judul_ar = judulAr || formData.judul_ar || '';
        formData.konten_ar = kontenAr || formData.konten_ar || '';
      }

      // Ensure user_id is set before insert
      ...
      toast.showSuccess('Berhasil', 'Pengumuman berhasil dibuat!');
      router.push('/admin/pengumuman');
    } catch (error: any) {
      console.error('Error creating pengumuman:', error);
      toast.showError('Error', error.message || 'Gagal membuat pengumuman');
    } finally {
      setSaving(false);
    }
  };
  ...
  if (authLoading) {
    return (
      <div>
        <PageBreadcrumb pageTitle="New Pengumuman" />
  ...
  return (
    <div>
      <PageBreadcrumb pageTitle="New Pengumuman" />
      <ComponentCard title="Create New Pengumuman">
