            ...
            <form
              onSubmit={async (e) => {
                ...
                if (error) {
                  ...
                  // If user is immediately logged in, redirect to admin
                  // Otherwise, show message about email verification
                  setTimeout(async () => {
                    const { data: { session } } = await supabase.auth.getSession();
                    if (session) {
                      router.push("/admin");
                    } else {
                      setError("Silakan cek email Anda untuk verifikasi akun sebelum login.");
                    }
                  }, 500);
                }
              }}
