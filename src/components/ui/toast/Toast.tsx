...
const Toast: React.FC<ToastProps> = ({
  ...
  const typeConfig = {
    success: {
      ...
      iconBg: 'bg-success-100 dark:bg-success-500/30',
    ...
    info: {
      ...
      titleColor: 'text-blue-light-800 dark:text-blue-light-300',
      icon: (
        <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 20 20">
  ...
  return (
    <div
      className={`flex items-start gap-4 p-4 rounded-xl border shadow-lg backdrop-blur-sm animate-slide-in-right ${config.bg} ${config.border} min-w-[320px] max-w-[500px]`}
      role="alert"
    >
      ...
      <div className="flex-1 min-w-0">
        <h4 className={`mb-1 text-sm font-semibold ${config.titleColor}`}>
          {title}
        </h4>
